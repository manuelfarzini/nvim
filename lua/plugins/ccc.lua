return {
  ...
  config = function()
    ...
    function HslRgbInput.to_rgb(value) return { value[4], value[5], value[6] } end
    ...
    opt.desc = "Pick color"
    vim.api.nvim_set_keymap("n", "<leader>cp", ":CccPick<CR>", opt)

    opt.desc = "Toggle color highlighter"
    vim.api.nvim_set_keymap("n", "<leader>ct", ":CccHighlighterToggle<CR>", opt)

    opt.desc = "Convert color"
    vim.api.nvim_set_keymap("n", "<leader>cn", ":CccConvert<CR>", opt)
    ...
    opt.desc = "Refresh color highlighter"
    vim.api.nvim_set_keymap("n", "<leader>cr", ":CccHighlighterToggleTwice<CR>", opt)
  end,
}
